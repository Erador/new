...
if ($choice -eq 1){
    ...
    Get-ChildItem -path $env:USERPROFILE -Filter *.txt | Where-Object {$_.lastwritetime -gt (Get-Date $cdate)} | Format-Table name, lastwritetime
    ...
    Write-Host "Invalid Input from $env:Computername by user: $env:USERNAME"
